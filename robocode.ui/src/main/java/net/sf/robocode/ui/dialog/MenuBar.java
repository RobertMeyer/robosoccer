 ...
 * Larsen - Code cleanup - Added menu items for Robocode API, Robo Wiki, Yahoo
 * Group Robocode, and Robocode Repository - Updated to use methods from the
 * WindowUtil, which replaces window methods that have been (re)moved from the
 * robocode.util.Utils class - Changed menu shortcut keys to use
 * getMenuShortcutKeyMask() instead of Event.CTRL_MASK in order to comply with
 * other OSes like e.g. Mac OS - Added "Recalculate CPU constant" to the Options
 * menu - Added "Clean Robot Cache" to the Options menu Matthew Reeder - Added
 * keyboard mnemonics and a few accelerators to all menus and menu items Luis
 * Crespo & Flemming N. Larsen - Added check box menu item for "Show Rankings"
 * *****************************************************************************
 */
...
public class MenuBar extends JMenuBar {
    ...
    private JMenuItem optionsGoToFullScreenModeMenuItem;
    ...
    private class EventHandler implements ActionListener, MenuListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            } else if (source == mb.getOptionsGoToFullScreenModeMenuItem()) {
                optionsGoToFullScreenModeActionPerformed();
    ...
    private JMenuItem getOptionsGoToFullScreenModeMenuItem() {
        if (optionsGoToFullScreenModeMenuItem == null) {
            optionsGoToFullScreenModeMenuItem = new JMenuItem();
            optionsGoToFullScreenModeMenuItem.setText("Full Screen");
            optionsGoToFullScreenModeMenuItem.setMnemonic('F');
            optionsGoToFullScreenModeMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_F11, 0));
            optionsGoToFullScreenModeMenuItem.addActionListener(eventHandler);
            FullScreenUtil.addFullScreenListener(new IFullScreenListener() {
                @Override
                public void fullScreenModeChanged(JFrame w, boolean isNowFullScreen) {
                    if (isNowFullScreen) {
                        // I'm in full screen mode, change the name to exit.
                        optionsGoToFullScreenModeMenuItem.setText("Exit Full Screen");
                    } else {
                        optionsGoToFullScreenModeMenuItem.setText("Full Screen");
                    }
                }
        ...
        return optionsGoToFullScreenModeMenuItem;
    }
    ...
    private JMenu getOptionsMenu() {
        if (optionsMenu == null) {
            ...
            optionsMenu.add(getOptionsGoToFullScreenModeMenuItem());
    ...
    private void optionsGoToFullScreenModeActionPerformed() {
        FullScreenUtil.toggleFullScreen(robocodeFrame);
    }

...
/**
 * Represents a set of equipment parts that can be used by battle logic to
 * ensure that Robots can only equip parts that are in this set.
 * 
 * @author Malcolm Inglis (CSSE2003 Team Forkbomb)
 */
public class EquipmentSpecification {
	private Map<String, EquipmentPart> parts;

	public static EquipmentSpecification fromFile(String path) {
		return new EquipmentSpecification(path);
	}
	
	private EquipmentSpecification(String path) {
		this.parts = new HashMap<String, EquipmentPart>();
		parts.put(

 ...
 * demonstrates a defensive style of play, where the robot constantly attempts
 * to position itself between the ball and it's own goal.
 * 
 * Author: Carl Hattenfels - team-G1
 * 
 */
public class DefensiveSoccerPlayer extends SoccerRobot {
	 ...
	 * 
	 * @param enemy
	 *            - The enemy that generated this event.
	 */
	...
	/**
	 * Linear targeting algorithm. Estimates enemies future position based on
	 * current velocity. Assumes that the enemy will stop when it hits the
	 * walls.
	 * 
	 * @param enemy
	 *            - event generated by Scanned Robot
	...
	public double aim(ScannedRobotEvent enemy) {
		 ...
		 * time for the enemy to reach a future position is approximately equal
		 * to the time it would take a bullet of given power to travel the
		 * distance between our current position and that future position.
		 */
		do {
			time += 0.1;
		 ...
		 */
		double adjustedBearing = (Math
				.atan2(futureX - getX(), futureY - getY()) - getGunHeadingRadians());
	 ...
	 * close the distance if he is too far away. Returns true if we are current
	 * positioned in a trajectory towards the goal.
	 * 
	 * @param enemy
	 *            - recently targeted enemy
	 */
	public boolean move(ScannedRobotEvent ball) {
		...
		double ballAbsoluteBearing = Utils
				.normalAbsoluteAngle(getHeadingRadians()
						+ ball.getBearingRadians());
		...
		/*
		 * We use the following lengths and points to draw a triangle. One apex
		 * of which originates at the balls position, and each side radiates
		 * outward at an angle away from our own goals top and bottom. We use
		 * this triangle as the bounds of the ideal positioning of our robot. In
		 * DefensiveRobot, by positioning our robot within the triangle, we have
		 * placed ourselves at some point directly between the ball and our own
		 * goals, increases our chances of obstructing the ball.
		 */
		double currentX = getX();
		double currentY = getY();
		int[] triPointsX = { (int) ballX, (int) (ownGoal.getCenterX()),
				(int) (ownGoal.getCenterX()) };
		int[] triPointsY = { (int) lastBallY, (int) (ownGoal.getMaxY()),
				(int) (ownGoal.getMinY()) };
		Polygon ownPolygon = new Polygon(triPointsX, triPointsY, 3);
		Point2D.Float us = new Point2D.Float((int) currentX, (int) currentY);

		/*
		 * This frankly terrible bit of code determines whether the polygon (our
		 * desired location on the field), is above or below us, so that we can
		 * move in the right direction to get to the polygon. There are many
		...
		for (int i = 0; i < battlefieldHeight; i += (battlefieldHeight / 40)) {
			if (ownPolygon.contains(currentX, currentY + i)) {
				polygonAbove = true;
			}
			if (ownPolygon.contains(currentX, currentY - i)) {
				polygonBelow = true;
		...
		/*
		 * The following code tries to keep us within the ideal field position,
		 * i.e. within the ownPolygon shape described above.
		 */
		if (!ownPolygon.contains(us)) {
			if (polygonAbove) {
		...
		return ownPolygon.contains(us);
	}

	/**
	 * Returns the absolute bearing from this robot to the supplied point, using
	...
	/**
	 * Returns the absolute bearing from this robot to its own goal, using
	 * robocode coordinate system.
	 */
	 ...
	 * 
	 * @param turn
	 *            - current heading chosen on the basis of our targets position
	 * @param side
	 *            - the side (left or right) of our bot the target is on.
	...
	/**
	 * Returns the robots heading based on direction of movement. i.e. if robot
	 * is moving backwards.
	 * 
	...
	/**
	 * Paints some useful debug graphics. 
	 */
	public void onPaint(Graphics2D g) {
		...
		g.fillRect((int) ownGoal.getMinX(), (int) ownGoal.getMinY(),
				(int) ownGoal.width, (int) ownGoal.height);

		// Draw enemy goal coloured Green
		g.setColor(new Color(0x00, 0xff, 0x00, 0x80));
		g.fillRect((int) enemyGoal.getMinX(), (int) enemyGoal.getMinY(),
				(int) enemyGoal.width, (int) enemyGoal.height);

		int[] triPointsX = { (int) lastBallX, (int) (ownGoal.getCenterX()),
				(int) (ownGoal.getCenterX()) };
		int[] triPointsY = { (int) lastBallY, (int) (ownGoal.getMaxY()),
				(int) (ownGoal.getMinY()) };
		...
		Line2D.ptSegDist(triPointsX[0], triPointsY[0], triPointsX[1],
				triPointsX[1], us.getX(), us.getY());
